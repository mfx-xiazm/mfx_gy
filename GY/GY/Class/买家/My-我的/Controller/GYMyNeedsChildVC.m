...
-(void)getTasksDataRequest:(BOOL)isRefresh
{
    ...
    parameters[@"task_status"] = @(self.task_status);//1 查询未接单 2查询已接单
    if (isRefresh) {
    ...
    hx_weakify(self);
    [HXNetworkTool POST:HXRC_M_URL action:@"getTaskList" parameters:parameters success:^(id responseObject) {
        hx_strongify(weakSelf);
